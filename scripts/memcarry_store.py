#!/usr/bin/env python3
"""memcarry: local-first shared L2 memory CLI.
...
MEMORY_HOME = Path(os.environ.get("MEMCARRY_HOME") or os.environ.get("AGENT_MEMORY_HUB_HOME") or "~/.memcarry").expanduser()
...
def root(args): return Path(args.home or MEMORY_HOME).expanduser()
...
def parser():
    p = argparse.ArgumentParser(prog="memcarry")
    p.add_argument("--home")
